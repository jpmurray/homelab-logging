...
GitHub Actions tests the project and publishes `homelab-logging-1.4.0-linux-amd64.zip`. Download that archive from the repository's Releases page and copy it to each Proxmox node.

On a node, keep releases under `/opt` and point `current` at the active one:

```bash
mkdir -p /opt/homelab-logging/releases
unzip homelab-logging-1.4.0-linux-amd64.zip -d /opt/homelab-logging/releases
ln -sfn /opt/homelab-logging/releases/homelab-logging-1.4.0 /opt/homelab-logging/current
